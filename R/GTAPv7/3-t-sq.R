...
sectors <- data.table::fread("./mappings/manufacturing.csv")
...
.data <- ems_data(
  ...
  COMM = "./mappings/manufacturing.csv",
  ACTS = "./mappings/manufacturing.csv",
  ENDW = "./mappings/labor_diff.csv"
)

# scenario
scenario <- "3-t"
forcings <- c(5, 1, 2.5, 7.5, 10) * -1
sfid <- "sq"
...
for (f in seq_along(forcings)) {
  ...
  # shocks
  afeall[ENDWe == "capital" &
    !ACTSa %in% c("svces", "food", "crops", "livestock") &
    REGr == "deu", Value := main_f]
